...
inline std::wstring get_attr_value(const std::wstring& xml, const std::wstring& tag, const std::wstring& attr)
{
    size_t tag_pos = xml.find(L"<" + tag);
    if (tag_pos == std::wstring::npos) return L"";

    size_t attr_pos = xml.find(attr + L"='", tag_pos);
    if (attr_pos == std::wstring::npos) return L"";
...
inline std::wstring get_data_value(const std::wstring& xml, const std::wstring& name)
{
...
inline std::wstring format_time(const std::wstring& iso)
{
    ...
    if (t_pos == std::wstring::npos) return iso;
...
inline time_t IsoToTimeT(const std::wstring& iso)
{
    ...
    swscanf_s(iso.c_str(), L"%4d-%2d-%2d %2d:%2d:%2d",
...
inline void FileInfoEvents()
{
    time_t logonTime = GetCurrentUserLogonTime();
    if (!logonTime) {
        std::wcerr << L"Could not get current user logon time.\n";
        return;
    }
    ...
    while (EvtNext(hQuery, 8, hEvents, INFINITE, 0, &returned)) {
        for (DWORD i = 0; i < returned; ++i) {
            ...
            EvtRender(nullptr, hEvent, EvtRenderEventXml, 0, nullptr, &used, &count);

            if (GetLastError() == ERROR_INSUFFICIENT_BUFFER) {
                std::vector<wchar_t> buffer(used);

                if (EvtRender(nullptr, hEvent, EvtRenderEventXml, used, buffer.data(), &used, &count)) {
                    ...
                    std::wcout << L"\n";
                    std::wcout << L"EventID    : " << ev.event_id << L"\n";
                    std::wcout << L"Time       : " << cleanTime << L"\n";
                    std::wcout << L"DeviceName : " << ev.device_name << L"\n";
                    std::wcout << L"Message    : " << ev.message << L"\n\n";
                }
            }
